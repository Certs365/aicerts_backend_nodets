 ...
 */
const getBadgeTemplateById = async (
  req: Request,
  res: Response
): Promise<void> => {
  const { id } = req.params;
 ...
 */
const deleteBadgeTemplates = async (
  req: Request,
  res: Response
): Promise<void> => {
  const { email } = req.body;
 ...
 */
const deleteBadgeTemplateById = async (
  req: Request,
  res: Response
): Promise<void> => {
  const { id } = req.params;
...
const addBadgeTemplate = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (!user) {
      res.status(400).json({
        status: 'FAILED',
        message: messageCodes.msgInvalidIssuer,
      });
      return;
    }
    ...
    res.json({
      status: 'SUCCESS',
      message: messageCodes.msgOperationSuccess,
      data: savedTemplate,
    });
  } catch (error) {
    console.error(messageCodes.msgInternalError, error);
    res.status(500).json({
      status: 'FAILED',
      message: messageCodes.msgInternalError,
    });
  }
};
