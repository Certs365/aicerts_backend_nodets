...
const transportConfig = {
  ...
  auth: {
    ...
    pass: process.env.SMTP_MAIL_PWD,
  },
};
...
export const sendGrievanceEmail = async (email: string, paymentID: string) => {
  try {
    const mailOptions = {
      from: from,
      to: 'Nikhil.Meshram@Aicerts.io',
      subject: 'Payment Grievance from user',
  ...
  <!--[if (mso)|(IE)]></td><![endif]-->
    ...
    logger.error(`Error: Failed to sent grievance mail`, error);
  }
};

//Email to user ,after buying any plan
export const planPurchasedEmail = async (
  email: string,
  name: string,
  userPlan: any
) => {
  ...
  <p style="margin-inline: 10px">Thank you for choosing us.</p>`;
    ...
    const mailOptions = {
      ...
      Plan Title: ${userPlan.subscriptionPlanTitle}
      Purchased Date: ${userPlan.purchaseDate}
      Subscription Duration: ${userPlan.subscriptionDuration} days
      Allocated Credentials: ${userPlan.allocatedCredentials}
      Subscription Fee: ${userPlan.subscriptionFee}
  ...
  <p style="margin: 0;">Your subscription to <em><strong>${userPlan.subscriptionPlanTitle} Plan </strong></em>has been successfully activated. Below are the details of your subscription:</p>
  </div>
  </td>
  </tr>
  </table>
  ...
  <!--[if !vml]><!-->
    ...
    logger.error(`Error: Failed to sent plan purchased mail`, error);
  }
};
...
export const sendOTPEmail = async (
  ...
  <!--[if !vml]><!-->
    ...
    logger.error(`Error: Failed to otp mail`, error);
  }
};
